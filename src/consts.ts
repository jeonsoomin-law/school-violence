 ...
 *         사무소 정보가 아직 가짜(02-000-0000, 홍길동)인 상태로 색인되는 것을 막습니다.
 * false = 검색엔진 공개.
 *
 * 아래 세 가지가 끝나면 false 로 바꾸고 다시 배포하세요.
 *   1. 사무소 실제 정보 입력 (아래 SITE / CONTACT / ADDRESS / ATTORNEY)
 *   2. 칼럼·해결사례 법률 내용 검토
...
/**
 * ⚠️ 첫 화면 자격 스트립에 노출되는 항목.
 * ─────────────────────────────────────────────
 * 아래 목록은 보내주신 위촉장 파일명에서 **추정**한 것입니다.
 * 공식 명칭·기관명이 실제와 다를 수 있고, 변호사 광고 규정상
 * 허위·과장으로 오인될 표현은 문제가 되므로 반드시 확인 후 확정해 주세요.
 *
 * 현재 위촉 중인 것과 과거 이력을 구분해야 한다면 label 에 "前" 을 붙이세요.
 * 항목을 지우려면 그냥 줄을 삭제하면 됩니다.
 */
export const CREDENTIALS = [
  { label: '교육부 위촉', detail: '학교폭력 관련 위원' }, // TODO: 정확한 위원회 명칭
  { label: '서울시교육청', detail: '학교폭력 사안처리지원단' },
  { label: '서울시교육청', detail: '교권보호위원회 위원' },
  { label: '대한변호사협회', detail: '연수 강사' },
  { label: '서울지방경찰청', detail: '감사장 수여' },
] as const;
