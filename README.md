...
분류 목록은 **관리자 화면 왼쪽 메뉴의 "분류 관리"** 에서 직접 추가·삭제·순서변경할 수 있습니다.
저장하면 글쓰기 화면의 분류 선택지가 바로 따라 바뀝니다.

**칼럼·해결사례·후기가 이 하나의 목록을 함께 씁니다.** 게시판별로 분류가 따로 있지 않습니다.
덕분에 같은 분류로 묶인 글이면 칼럼이든 해결사례든 후기든 서로 연결됩니다.

실제 목록은 `src/data/categories.json` 에 저장됩니다. 세 목록 페이지의 분류 노출 순서도
여기서 정한 순서를 그대로 따릅니다. 글이 하나도 없는 분류는 목록에 나타나지 않습니다.

### 같은 분류 글끼리 연결되는 방식

- **목록 페이지** — 칼럼·해결사례·후기 모두 분류별로 묶여서 표시되고, 맨 위에 분류 바로가기가 붙습니다.
- **글 하단** — 칼럼과 해결사례 상세 페이지 아래에 같은 분류의 다른 글이 최대 6개 붙습니다.
  칼럼·해결사례·후기를 번갈아 담아 한 종류로만 채워지지 않게 합니다.
- **같은 분류 글이 하나도 없을 때** — 영역이 사라지면 내부링크가 끊기므로, 목표 검색어가 겹치는
  다른 글 3개로 채우고 제목을 "함께 읽으면 좋은 글" 로 바꿉니다.
- **후기** — 개별 페이지가 없어서 목록 페이지의 해당 후기 위치(`/review/#rv-주소`)로 연결됩니다.

> **이미 쓰고 있는 분류를 지우거나 이름을 바꿀 때**는 그 분류로 써둔 글의 분류도 함께 고쳐주세요.
> 안 고쳐도 사이트는 정상 배포되고 그 글도 그대로 보이지만(맨 아래 묶음으로 내려갑니다),
> 배포 로그에 `[분류 확인]` 경고가 남습니다.
