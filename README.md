...
칼럼·해결사례·후기에서 고르는 분류 목록은 **관리자 화면 왼쪽 메뉴의 "분류 관리"** 에서 직접
추가·삭제·순서변경할 수 있습니다. 저장하면 글쓰기 화면의 분류 선택지가 바로 따라 바뀝니다.

실제 목록은 `src/data/categories.json` 에 저장됩니다. 칼럼 목록 페이지(`/column/`)의 분류
노출 순서도 여기서 정한 순서를 그대로 따릅니다. 글이 하나도 없는 분류는 목록에 나타나지 않습니다.
